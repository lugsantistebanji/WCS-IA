...
![](https://imgur.com/rnRgSMZ.png)


- L’entraîneur des Gryffindor est superstitieux, son jour préféré est le lundi. Retourne la liste des joueurs de son équipe qui ont été enrôlés un lundi (il souhaite les faire jouer en priorité... si seulement il existait une fonction SQL pour ça...), et classe les résultats par date d’enrôlement chronologique.

```SQL
SELECT 
    wizard.firstname AS firstname,
    wizard.lastname AS lastname,
    player.enrollment_date AS enrollment_date 
FROM wizard 
INNER JOIN player
...
WHERE
    ...
    AND 
    DAYNAME(player.enrollment_date) = 'Monday'
ORDER BY 
    player.enrollment_date DESC
;
```

![](https://imgur.com/XayOyKr.png)
